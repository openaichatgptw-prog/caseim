"""
ventas_precios_cnh.py  —  version optimizada
=============================================
Extrae el detalle de ventas desde SQL Server (UnoEE) desde el 01/01 del año
(año_actual - 2), normaliza la columna Referencia para cruzarla con el
pipeline de precios CNH almacenado en DuckDB, agrega la columna "Precio CNH"
y exporta el resultado a Excel y CSV.

Optimizaciones aplicadas:
  - pd.read_sql con chunksize  : lectura segura con tipos correctos
  - pd.concat con copy=False   : evita copia extra en memoria
  - duck.register()            : pasa DataFrame como vista Arrow sin reinferir tipos
  - xlsxwriter                 : hasta 5x mas rapido que openpyxl para Excel
  - optimizar_tipos()          : downcast de int64/float64 reduce ~30% de RAM

Dependencias: pandas, pyodbc, duckdb, xlsxwriter
Config:       config.ini (secciones SQLSERVER, SALIDA, SALIDA_VENTAS)
...
# Lee credenciales y rutas desde config.ini.
# Calcula dinamicamente la fecha de inicio:
#   fecha_desde = 01/01 del (año_actual - 2)
#   Ejemplo: año 2026 → fecha_desde = 2024-01-01
# ===========================================================================
...
# Genera el fragmento SQL que estandariza una referencia de item para que
# el cruce entre ventas (SQL Server) y precios (DuckDB) sea consistente.
# Pasos que aplica sobre el campo recibido:
#   1. Convierte a VARCHAR y elimina tabulaciones, saltos de linea y retornos
#   2. Reemplaza guion bajo por guion medio
#   3. Elimina caracteres especiales (conserva A-Z, 0-9, punto, guion, barra)
#   4. Elimina puntos al inicio y al final
#   5. Colapsa puntos consecutivos en uno solo
#   6. Colapsa guiones consecutivos en uno solo
#   7. Elimina espacios alrededor de guiones
#   8. Colapsa espacios multiples en uno solo
#   9. Convierte todo a mayusculas y elimina espacios extremos
# ===========================================================================
...
# Detalle de ventas desde fecha_desde hasta el dia anterior al actual.
# Incluye datos de factura, cliente, vendedor, item, costos, precios lista,
# rotacion, sistema de precio y margen objetivo por sistema.
#
# Tablas principales:
#   t461  Encabezado factura de venta  → fecha, cliente, vendedor
#   t460  Remision asociada a factura
#   t350  Documento contable           → CO, tipo, consecutivo
#   t470  Movimiento de inventario     → cantidades, valores, costos, lista
#   t120  Maestro de items             → referencia, descripcion, tipo
#   t121  Extension de items           → llave de union a movimiento
#   t150  Bodegas                      → bodega y lista sugerida
#   t157  Instalaciones
#   t112  Listas de precios
#   t146  Motivos de movimiento
#   t200  Terceros                     → cliente y vendedor (alias distintos)
#   t281  Unidades de negocio
#   nco   Vista AFAS_NOMBRECO          → descripcion del CO
#
# Subconsultas LEFT JOIN:
#   rot   Rotacion del item (plan 08)  → Alta / Baja
#   sist  Sistema de precios (plan 12) → codigo y descripcion
#   lp    Ultimo precio activo lista 04 (BEMER) o 09 (resto)
#   obj   Margen objetivo por sistema de precio (entidad UnoEE)
#   t430  Pedido PV asociado al movimiento
#
# Logica de cantidades y valores:
#   f470_ind_naturaleza = 2  → venta normal   (signo positivo)
#   f470_ind_naturaleza != 2 → devolucion     (signo negativo)
#
# Filtros principales:
#   - f461_id_fecha >= fecha_desde (parametro dinamico año - 2)
#   - f461_id_fecha <  primer dia del mes actual
#   - f350_ind_estado = 1 (documentos activos/legalizados)
#   - f470_id_cia = 1 (compania 1)
#   - f125_id_plan = '07' (clasificacion linea del item)
# ===========================================================================
def build_query_ventas(fecha_desde: str) -> str:
    ...
    FROM t460_cm_docto_remision_venta
        ...
        -- Subconsulta: rotacion del item segun clasificacion plan 08
        LEFT JOIN (
            SELECT DISTINCT
                t120_mc_items.f120_rowid,
                t106b.f106_id
        ...
        -- Subconsulta: sistema de precios del item segun clasificacion plan 12
        LEFT JOIN (
            SELECT DISTINCT
                t120_mc_items.f120_rowid,
                t106c.f106_id,
        ...
        -- Subconsulta: ultimo precio activo de lista 04 (BEMER) o 09 (resto)
        -- Filtra por la fecha de activacion mas reciente por referencia
        LEFT JOIN (
            SELECT DISTINCT
            ...
            AND lp.f126_id_lista_precio = CASE WHEN t150_mc_bodegas.f150_id = 'BEMER'
                                               THEN '04' ELSE '09' END
        -- Subconsulta: margen objetivo por sistema de precio (entidad UnoEE CO 001)
        LEFT JOIN (
            SELECT
...
# ===========================================================================
# SECCION 4 - EXTRACCION DE VENTAS
# pd.read_sql garantiza conversion correcta de tipos DECIMAL de SQL Server
# a float64 de pandas — evita problemas de tipo en DuckDB.
# Lee en bloques de 50.000 filas para no saturar memoria.
# Aplica downcast de tipos al final para reducir ~30% de RAM.
# ===========================================================================
...
# ===========================================================================
# SECCION 5 - CRUCE CON PRECIO CNH
# duck.register() pasa el DataFrame como vista Arrow respetando los tipos
# de pandas sin reinferencia — unica forma segura con columnas DECIMAL.
# norm_sql() se aplica sobre [Referencia] para garantizar el cruce correcto
# con Referencia_Normalizada del pipeline de precios CNH.
# Precio CNH = Precio Prorrateo de resultado_precios_lista en DuckDB.
# ===========================================================================
def cruzar_con_precio_cnh(df_ventas: pd.DataFrame) -> pd.DataFrame:
    ...
    norm_ref = norm_sql('v."Referencia"')
    ...
    # Reubica Ref_Normalizada junto a Referencia para mejor legibilidad
...
# xlsxwriter es hasta 5x mas rapido que openpyxl para archivos grandes.
# CSV con encoding utf-8-sig para compatibilidad con Excel en Windows.
# ===========================================================================
